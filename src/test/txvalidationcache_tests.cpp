...
BOOST_FIXTURE_TEST_CASE(checkinputs_test, Dersig100Setup)
{
    ...
    // spend_tx is valid according to DERSIG
    {
        ...
        ValidateCheckInputsForAllFlags(CTransaction(spend_tx), 0, false, m_node.chainman->ActiveChainstate().CoinsTip());
    }

    // And if we produce a block with this tx, it should be valid (DERSIG enabled)
    ...
    // for the same tx with a different witness.
    {
        ...
        valid_with_witness_tx.nVersion = 2;
