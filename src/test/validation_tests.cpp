...
{
    int maxHalvings = 7;
    CAmount nInitialSubsidy = 2000 * COIN;
    ...
    // Calculate and test subsidy at the end height where the subsidy remains constant
    int endHeight = maxHalvings * consensusParams.nSubsidyHalvingInterval + 1051000;
    BOOST_CHECK_EQUAL(GetBlockSubsidy(endHeight, consensusParams), 1607500000);

    // Check subsidy becomes zero after 28,380,500 blocks
    BOOST_CHECK_EQUAL(GetBlockSubsidy(endHeight + 19972500, consensusParams), 0);
}
...
BOOST_AUTO_TEST_CASE(subsidy_limit_test)
{
    const auto chainParams = CreateChainParams(ChainType::MAIN);
    CAmount nSum = 0;
    for (int nHeight = 0; nHeight < 28403000; nHeight += 1000) {
        CAmount nSubsidy = GetBlockSubsidy(nHeight, chainParams->GetConsensus());
        BOOST_CHECK(nSubsidy <= 2000 * COIN);
        nSum += nSubsidy * 1000;
        BOOST_CHECK(MoneyRange(nSum));
    }
    BOOST_CHECK_EQUAL(nSum, CAmount{450000000000000000});
}
...
BOOST_AUTO_TEST_CASE(test_assumeutxo)
{
    ...
    BOOST_CHECK_EQUAL(out110.hash_serialized.ToString(), "e78546efddc5f8b276425c147b45cfed5a2b1e02b71c765523e1b66de5931cf9");
    BOOST_CHECK_EQUAL(out110.nChainTx, 111U);

    const auto out110_2 = *params->AssumeutxoForBlockhash(uint256S("0x45f36119711daa114853525f53d83f532b6a1246a92dc5454518e7d51bbe15bf"));
    BOOST_CHECK_EQUAL(out110_2.hash_serialized.ToString(), "e78546efddc5f8b276425c147b45cfed5a2b1e02b71c765523e1b66de5931cf9");
