...
std::shared_ptr<CBlock> MinerTestingSetup::FinalizeBlock(std::shared_ptr<CBlock> pblock)
{
    ...
    m_node.chainman->GenerateCoinbaseCommitment(*pblock, prev_block, Params().GetConsensus());

    pblock->hashMerkleRoot = BlockMerkleRoot(*pblock);

    uint256 hashMix;
    while (!CheckProofOfWork(pblock->GetHash(hashMix), pblock->nBits, Params().GetConsensus())) {
        ++(pblock->nNonce);
    }
