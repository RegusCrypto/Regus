...
#include <crypto/ethash/lib/ethash/primes.h>
...
inline hash256 hash_final(const hash512& seed, const hash256& hashMix)
{
    uint8_t final_data[sizeof(seed) + sizeof(hashMix)];
    std::memcpy(&final_data[0], seed.bytes, sizeof(seed));
    std::memcpy(&final_data[sizeof(seed)], hashMix.bytes, sizeof(hashMix));
    ...
    const epoch_context& context, const hash512& seed, lookup_fn lookup) noexcept
{
    ...
    for (size_t i = 0; i < num_words; i += 4)
    {
        ...
        hashMix.word32s[i / 4] = h3;
    }

    return le::uint32s(hashMix);
}
}  // namespace
...
result hash(const epoch_context_full& context, const hash256& header_hash, uint64_t nonce) noexcept
{
    ...
    const hash256 hashMix = hash_kernel(context, seed, lazy_lookup);
    return {hash_final(seed, hashMix), hashMix};
}
...
{
    const hash512 seed = hash_seed(*header_hash, nonce);
    const hash256 hashMix = hash_kernel(*context, seed, calculate_dataset_item_1024);
    return {hash_final(seed, hashMix), hashMix};
}

bool ethash_verify_final_hash(const hash256* header_hash, const hash256* hashMix, uint64_t nonce,
    const hash256* boundary) noexcept
{
    const hash512 seed = hash_seed(*header_hash, nonce);
    return is_less_or_equal(hash_final(seed, *hashMix), *boundary);
}
...
{
    const hash512 seed = hash_seed(*header_hash, nonce);
    if (!is_less_or_equal(hash_final(seed, *hashMix), *boundary))
        return false;

    const hash256 expected_hashMix = hash_kernel(*context, seed, calculate_dataset_item_1024);
    return is_equal(expected_hashMix, *hashMix);
}
