...
struct search_result
{
    ...
    search_result(result res, uint64_t n) noexcept
      : solution_found(true), nonce(n), final_hash(res.final_hash), hashMix(res.hashMix)
    {}
};
...
inline bool verify_final_hash(const hash256& header_hash, const hash256& hashMix, uint64_t nonce,
    const hash256& boundary) noexcept
{
    return ethash_verify_final_hash(&header_hash, &hashMix, nonce, &boundary);
}

inline bool verify(const epoch_context& context, const hash256& header_hash, const hash256& hashMix,
    uint64_t nonce, const hash256& boundary) noexcept
{
    return ethash_verify(&context, &header_hash, &hashMix, nonce, &boundary);
}
