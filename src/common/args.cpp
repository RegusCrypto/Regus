...
const char * const REGUS_CONF_FILENAME = "regus.conf";
const char * const REGUS_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //regus-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", REGUS_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Regus
    // macOS: ~/Library/Application Support/Regus
    // Unix-like: ~/.regus
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Regus";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Regus";
#else
    // Unix-like
    return pathRet / ".regus";
#endif
#endif
}
