...
/** Call node.updateRwSetting() with Regus 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Regus 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/RegusCrypto/Regus/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because regus 22.x will not try to read these.
...
//! Convert enabled/size values to regus -prune setting.
...
//! Get pruning enabled value to show in GUI from regus -prune setting.
...
//! Get pruning size value to show in GUI from regus -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayRegusUnit")) {
        settings.setValue("DisplayRegusUnit", QVariant::fromValue(RegusUnit::REG));
    }
    QVariant unit = settings.value("DisplayRegusUnit");
    if (unit.canConvert<RegusUnit>()) {
        m_display_regus_unit = unit.value<RegusUnit>();
    } else {
        m_display_regus_unit = RegusUnit::REG;
        settings.setValue("DisplayRegusUnit", QVariant::fromValue(m_display_regus_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if regus.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_regus_unit);
...
{
    if (new_unit.isNull() || new_unit.value<RegusUnit>() == m_display_regus_unit) return;
    m_display_regus_unit = new_unit.value<RegusUnit>();
    QSettings settings;
    settings.setValue("DisplayRegusUnit", QVariant::fromValue(m_display_regus_unit));
    Q_EMIT displayUnitChanged(m_display_regus_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/RegusCrypto/Regus/pull/8273
    ...
    // (https://github.com/RegusCrypto/gui/issues/567).
