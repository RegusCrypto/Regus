...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<RegusUnit>("RegusUnit");
#else
    qRegisterMetaType<RegusUnit>("RegusUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. regus_de.qm (shortcut "de" needs to be defined in regus.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. regus_de_DE.qm (shortcut "de_DE" needs to be defined in regus.qrc)
...
static const char* qt_argv = "regus-qt";

RegusApplication::RegusApplication()
...
void RegusApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the RegusApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", RegusGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void RegusApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool RegusApplication::createOptionsModel(bool resetSettings)
{
...
void RegusApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new RegusGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &RegusGUI::quitRequested, this, &RegusApplication::requestShutdown);
...
void RegusApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void RegusApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool RegusApplication::baseInitialize()
{
    return node().baseInitialize();
}

void RegusApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &RegusApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &RegusApplication::handleRunawayException);
    connect(this, &RegusApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &RegusApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void RegusApplication::parameterSetup()
{
...
void RegusApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void RegusApplication::requestInitialize()
{
...
void RegusApplication::requestShutdown()
{
...
void RegusApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // regus: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &RegusGUI::handlePaymentRequest);
            connect(window, &RegusGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void RegusApplication::handleRunawayException(const QString &message)
{
...
void RegusApplication::handleNonFatalException(const QString& message)
{
...
WId RegusApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool RegusApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", RegusGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(regus);
    Q_INIT_RESOURCE(regus_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(REGUS_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see regus-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, PACKAGE_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see regus-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse regus.conf, determine network, switch to network specific
    ...
    // regus: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
