                  ...
                  QMessageBox::StandardButton confirm_type = QMessageBox::Yes)
{
    ...
    entry->findChild<RegusAmountField*>("payAmount")->setValue(amount);
...
void CompareBalance(WalletModel& walletModel, CAmount expected_balance, QLabel* balance_label_to_check)
{
    ...
    QString balanceComparison = RegusUnits::formatWithUnit(unit, expected_balance, false, RegusUnits::SeparatorStyle::ALWAYS);
...
std::shared_ptr<CWallet> SetupLegacyWallet(interfaces::Node& node, TestChain100Setup& test)
{
    ...
    wallet->LoadWallet();
    {
        ...
        CTxDestination dest = GetDestinationForKey(test.coinbaseKey.GetPubKey(), OutputType::LEGACY);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_regus-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_regus-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_regus-qt  # macOS
void TestGUI(interfaces::Node& node, const std::shared_ptr<CWallet>& wallet)
{
    ...
    // Amount input
    RegusAmountField* amountInput = receiveCoinsDialog.findChild<RegusAmountField*>("reqAmount");
...
void TestLegacyGUI(interfaces::Node& node)
{
    // Set up wallet and chain with 105 blocks (5 mature blocks for spending).
    TestChain100Setup legacyTest;
    for (int i = 0; i < 5; ++i) {
        legacyTest.CreateAndProcessBlock({}, GetScriptForRawPubKey(legacyTest.coinbaseKey.GetPubKey()));
    }
    auto wallet_loader = interfaces::MakeWalletLoader(*legacyTest.m_node.chain, *Assert(legacyTest.m_node.args));
    legacyTest.m_node.wallet_loader = wallet_loader.get();
    node.setContext(&legacyTest.m_node);

    // "Full" GUI tests, using legacy wallets
    const std::shared_ptr<CWallet>& legacy_wallet = SetupLegacyWallet(node, legacyTest);
    TestGUI(node, legacy_wallet);
}

void TestDescriptorGUI(interfaces::Node& node)
{
    // Set up wallet and chain with 105 blocks (5 mature blocks for spending).
    TestChain100Setup descriptorTest;
    for (int i = 0; i < 5; ++i) {
        descriptorTest.CreateAndProcessBlock({}, GetScriptForRawPubKey(descriptorTest.coinbaseKey.GetPubKey()));
    }
    auto wallet_loader = interfaces::MakeWalletLoader(*descriptorTest.m_node.chain, *Assert(descriptorTest.m_node.args));
    descriptorTest.m_node.wallet_loader = wallet_loader.get();
    node.setContext(&descriptorTest.m_node);

    // "Full" GUI tests, using descriptor wallets
    const std::shared_ptr<CWallet>& desc_wallet = SetupDescriptorsWallet(node, descriptorTest);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_regus-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestLegacyGUI(m_node);
    TestDescriptorGUI(m_node);
}
