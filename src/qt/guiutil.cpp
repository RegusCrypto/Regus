...
#include <qt/regusaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Regus address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new RegusAddressEntryValidator(parent));
    widget->setCheckValidator(new RegusAddressCheckValidator(parent));
}
...
bool parseRegusURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no regus: URI
    if(!uri.isValid() || uri.scheme() != QString("regus"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!RegusUnits::parse(RegusUnit::REG, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseRegusURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseRegusURI(uriInstance, out);
}

QString formatRegusURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("regus:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(RegusUnits::format(RegusUnit::REG, info.amount, false, RegusUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openRegusConf()
{
    ...
    /* Open regus.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Regus.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Regus (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Regus (%s).lnk", ChainTypeToString(chain)));
}
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "regus.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("regus-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a regus.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=Regus\n";
        else
            optionFile << strprintf("Name=Regus (%s)\n", ChainTypeToString(chain));
