This directory contains the source code for the Regus Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.

The current precise version for Qt 5 is specified in [qt.mk](/depends/packages/qt.mk).

## Compile and run

See build instructions: [Unix](/docs/build-unix.md), [macOS](/docs/build-osx.md), [Windows](/docs/build-windows.md), [FreeBSD](/docs/build-freebsd.md), [NetBSD](/docs/build-netbsd.md), [OpenBSD](/docs/build-openbsd.md)
...
- Contains translations. They are periodically updated and an effort is made to support as many languages as possible. The process of contributing translations is described in [docs/translation_process.md](/docs/translation_process.md).
...
- Represents the main window of the Regus UI.
...
- ClientModel is used by the main application `regusgui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a regus:... link from a browser).
...
* UI elements like RegusAmountField, which inherit from QWidget.
* `regusstrings.cpp`: automatically generated
* `regusunits.(h/cpp)`: REG / mREG / etc. handling
* `callback.h`
...
**Note:** Do not change `local/regus_en.ts`. It is updated [automatically](/docs/translation_process.md#writing-code-with-translations).
...
4. Enter `regus-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `regus-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `regus-qt`
9. You're all set! Start developing, building, and debugging the Regus Core GUI
