...
bool Intro::showIfNeeded(bool& did_show_intro, int64_t& prune_MiB)
{
    ...
    if(!fs::exists(GUIUtil::QStringToPath(dataDir)) || gArgs.GetBoolArg("-choosedatadir", DEFAULT_CHOOSE_DATADIR) || settings.value("fReset", false).toBool() || gArgs.GetBoolArg("-resetguisettings", false))
    {
        ...
        intro.setWindowIcon(QIcon(":icons/regus"));
        did_show_intro = true;
     ...
     * override -datadir in the regus.conf file in the default data directory
     * (to be consistent with regusd behavior)
     */
    if(dataDir != GUIUtil::getDefaultDataDirectory()) {
...
void Intro::UpdatePruneLabels(bool prune_checked)
{
    ...
    static constexpr uint64_t nPowTargetSpacing = 1 * 60;  // from chainparams, which we don't have at this stage
    static constexpr uint32_t expected_block_data_size = 15000;  // includes undo data
    ...
    ui->sizeWarningLabel->setText(
        tr("%1 will download and store a copy of the Regus block chain.").arg(PACKAGE_NAME) + " " +
