...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (m_pool.m_require_standard && !AreInputsStandard(tx, m_view, Consensus::Params().TaprootActive)) {
    ...
    if (!ancestors) {
        ...
        // this, see https://lists.linuxfoundation.org/pipermail/regus-dev/2018-November/016518.html
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    // After the seventh halving, the subsidy is kept at 15.625 for 19,972,500 blocks
    if (halvings >= 7) {
        if (nHeight >= 28380500) return 0 * COIN;

        return 15.625 * COIN;
    }

    if (nHeight <= 10) return 4500000 * COIN;

    CAmount nSubsidy = 2000 * COIN;
    // Subsidy is cut in half every 1,051,000 blocks until the seventh halving.
    nSubsidy >>= halvings;
    return nSubsidy;
}
...
static unsigned int GetBlockScriptFlags(const CBlockIndex& block_index, const ChainstateManager& chainman)
{
    ...
    // Enforce BIP16 (P2SH)
    if (consensusParams.BIP16Active) {
        flags |= SCRIPT_VERIFY_P2SH;
    }

    // Enforce Segwit (WITNESS)
    if (consensusParams.BIP16Active && consensusParams.SegwitActive) {
        flags |= SCRIPT_VERIFY_WITNESS;
    }

    // Enforce BIP65 (CHECKLOCKTIMEVERIFY)
    if (consensusParams.BIP65Active) {
        flags |= SCRIPT_VERIFY_CHECKLOCKTIMEVERIFY;
    }

    // Enforce BIP66 (DERSIG)
    if (consensusParams.BIP66Active) {
        flags |= SCRIPT_VERIFY_DERSIG;
    }

    // Enforce BIP112 (CHECKSEQUENCEVERIFY)
    if (consensusParams.CSVActive) {
        flags |= SCRIPT_VERIFY_CHECKSEQUENCEVERIFY;
    }

    // Enforce Taproot
    if (consensusParams.TaprootActive) {
        flags |= SCRIPT_VERIFY_TAPROOT;
    }

    // Enforce BIP147 (NULLDUMMY) if Segwit (WITNESS) is active
    if (consensusParams.SegwitActive) {
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    uint256 hashMix;
    uint256 block_hash{block.GetHash(hashMix)};
    ...
    if (params.GetConsensus().CSVActive) {
...
void ChainstateManager::ReceivedBlockTransactions(const CBlock& block, CBlockIndex* pindexNew, const FlatFilePos& pos)
{
    ...
    if (this->GetConsensus().SegwitActive) {
...
{
    uint256 hashMix;
    if (fCheckPOW && !CheckProofOfWork(block.GetHash(hashMix), block.nBits, consensusParams))
        return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "high-hash", "proof of work failed");
        
    if (fCheckPOW && hashMix != block.hashMix)
        return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "invalid-hash-mix", "hashMix validity failed");
...
void ChainstateManager::UpdateUncommittedBlockStructures(CBlock& block, const CBlockIndex* pindexPrev, const Consensus::Params& consensusParams) const
{
    int commitpos = GetWitnessCommitmentIndex(block);
    static const std::vector<unsigned char> nonce(32, 0x00);
    if (commitpos != NO_WITNESS_COMMITMENT && consensusParams.SegwitActive && !block.vtx[0]->HasWitness()) {
...
std::vector<unsigned char> ChainstateManager::GenerateCoinbaseCommitment(CBlock& block, const CBlockIndex* pindexPrev, const Consensus::Params& consensusParams) const
{
...
bool IsBlockMutated(const CBlock& block, bool check_witness_root)
{
    ...
    if (block.vtx.empty() || !block.vtx[0]->IsCoinBase()) {
        ...
        // in "Weaknesses in Regus’s Merkle Root Construction":
        // https://lists.linuxfoundation.org/pipermail/regus-dev/attachments/20190225/a27d8837/attachment-0001.pdf).
        //
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, BlockManager& blockman, const ChainstateManager& chainman, const CBlockIndex* pindexPrev) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    if ((block.nVersion < 2 && consensusParams.BIP34Active) ||
        (block.nVersion < 3 && consensusParams.BIP66Active) ||
        (block.nVersion < 4 && consensusParams.BIP65Active)) {
...
static bool ContextualCheckBlock(const CBlock& block, BlockValidationState& state, const ChainstateManager& chainman, const CBlockIndex* pindexPrev)
{
    ...
    if (consensusParams.BIP34Active)
    {
    ...
    if (!CheckWitnessMalleation(block, consensusParams.SegwitActive, state)) {
        return false;
    }
...
bool ChainstateManager::ProcessNewBlock(const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, bool* new_block)
{
...

    {
        ...
        // https://lists.linuxfoundation.org/pipermail/regus-dev/2019-February/016697.html.  Because CheckBlock() is
...
bool Chainstate::NeedsRedownload() const
{
    ...
    // If m_params.SegwitActive = true, segwit consensus rules must be validated
    CBlockIndex* block{m_chain.Tip()};

    while (block != nullptr && m_chainman.GetConsensus().SegwitActive) {
