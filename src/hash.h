...
/** A hasher class for Regus's 256-bit hash (double SHA-256). */
class CHash256 {
private:
    CSHA256 sha;
public:
...
/** A hasher class for Regus's 160-bit hash (SHA-256 + RIPEMD-160). */
class CHash160 {
private:
    CSHA256 sha;
public:
...
/** ETHash hashing function, returns only hash */
uint256 ETHash(const CBlockHeader& blockHeader);

/** ETHash hashing function, returns the hash and hashMix */
