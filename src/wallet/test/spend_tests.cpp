...
BOOST_FIXTURE_TEST_CASE(SubtractFee, TestChain100Setup)
{
    ...
    auto check_tx = [&wallet](CAmount leftover_input_amount) {
        CRecipient recipient{PubKeyDestination({}), 2000 * COIN - leftover_input_amount, /*subtract_fee=*/true};
...
BOOST_FIXTURE_TEST_CASE(wallet_duplicated_preset_inputs_test, TestChain100Setup)
{
    ...
    // Add 4 spendable UTXO, 2000 REG each, to the wallet (total balance 8000 REG)
    ...
    // Preselect the first 3 UTXO (6000 REG total)
    std::set<COutPoint> preset_inputs = {coins[0].outpoint, coins[1].outpoint, coins[2].outpoint};

    // Try to create a tx that spends more than what preset inputs + wallet selected inputs are covering for.
    // The wallet can cover up to 8000 REG, and the tx target is 9000 REG.
    std::vector<CRecipient> recipients{{*Assert(wallet->GetNewDestination(OutputType::BECH32, "dummy")),
                                           /*nAmount=*/9000 * COIN, /*fSubtractFeeFromAmount=*/true}};
    CCoinControl coin_control;
    ...
    // Attempt to send 9000 REG from a wallet that only has 8000 REG. The wallet should exclude
    // the preset inputs from the pool of available coins, realize that there is not enough
    // money to fund the 9000 REG payment, and fail with "Insufficient funds".
    //
    // Even with SFFO, the wallet can only afford to send 8000 REG.
    // If the wallet does not properly exclude preset inputs from the pool of available coins
    // prior to coin selection, it may create a transaction that does not fund the full payment
    // amount or, through SFFO, incorrectly reduce the recipient's amount by the difference
    // between the original target and the wrongly counted inputs (in this case 1000 REG)
    // so that the recipient's amount is no longer equal to the user's selected target of 9000 REG.
