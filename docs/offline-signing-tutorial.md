...
This tutorial will describe how to use two instances of Regus Core, one online and one offline, to greatly increase security by not having private keys reside on a networked device.
...
This workflow uses [Partially Signed Regus Transactions](https://github.com/RegusCrypto/Regus/blob/master/docs/psbt.md) (PSBTs) to transfer the transaction to and from the offline wallet for signing using the private keys.

> [!NOTE]
> While this tutorial demonstrates the process using `testnet` network, you should omit the `-testnet` flag in the provided commands when working with `mainnet`.

## Overview
In this tutorial we have two hosts, both running Regus v25.0
...
```sh
[offline]$ ./src/regus-cli -testnet -named createwallet \
...
```sh
[offline]$ ./src/regus-cli -testnet -rpcwallet="offline_wallet" listdescriptors \
             | jq -r '.descriptors' \
...
```sh
[online]$ ./src/regus-cli -testnet -named createwallet \
...
```sh
[online]$ ./src/regus-cli -testnet -rpcwallet="watch_only_wallet" importdescriptors "$(cat /path/to/descriptors.json)"
...
```sh
[online]$ ./src/regus-cli -testnet -rpcwallet="watch_only_wallet" getnewaddress

tb1qtu5qgc6ddhmqm5yqjvhg83qgk2t4ewajg0h6yh
```

2. Visit a faucet like https://testnetfaucet.com and enter your address from the previous command to receive a small amount of testnet coins to this address.
...
```sh
[online]$ ./src/regus-cli -testnet -rpcwallet="watch_only_wallet" listunspent
...
```sh
[online]$ ./src/regus-cli -testnet -rpcwallet="watch_only_wallet" send \
...
```sh
[offline]$ ./src/regus-cli -testnet decodepsbt $(cat /path/to/funded_psbt.txt)

{
    ...
}

[offline]$ ./src/regus-cli -testnet analyzepsbt $(cat /path/to/funded_psbt.txt)
...
```sh
[offline]$ ./src/regus-cli -testnet -rpcwallet="offline_wallet" walletpassphrase "** enter passphrase **" 60
```
...
2. Process, sign and finalize the PSBT on the `offline_wallet` using the `walletprocesspsbt` command, saving the output to a file `final_psbt.txt`.
 ...
 ```sh
[offline]$ ./src/regus-cli -testnet -rpcwallet="offline_wallet" walletprocesspsbt \
...
```sh
[online]$ ./src/regus-cli -testnet sendrawtransaction $(cat /path/to/final_psbt.txt)
...
```sh
[online]$ ./src/regus-cli -testnet -rpcwallet="watch_only_wallet" getbalances
...
```sh
[online]$ ./src/regus-cli -testnet -rpcwallet="watch_only_wallet" listtransactions
