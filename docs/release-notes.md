Regus Core version 27.0 is now available from:

  <https://regus.app/bin/regus-core-27.0/>
...
Please report bugs using the issue tracker at GitHub:
  ...
  <https://github.com/RegusCrypto/Regus/issues>

To receive security and update notifications, please subscribe to:

  <https://regus.app/en/list/announcements/join/>
...
installer (on Windows) or just copy over `/Applications/Regus-Qt` (on macOS)
or `regusd`/`regus-qt` (on Linux).

Upgrading directly from a version of Regus Core that has reached its EOL is
possible, but it might take some time if the data directory needs to be migrated. Old
wallet versions of Regus Core are generally supported.

Compatibility
==============

Regus Core is supported and extensively tested on operating systems
using the Linux Kernel 3.17+, macOS 11.0+, and Windows 7 and newer. Regus
Core should also work on most other Unix-like systems but is not as
frequently tested on them. It is not recommended to use Regus Core on
unsupported systems.
...
- A C++20 capable compiler is now required to build Regus Core. (#28349)
...
- Various fixes to prevent issues where subsequent instances of Regus Core would
...
[Transifex](https://www.transifex.com/regus/regus/).
