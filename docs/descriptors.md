# Support for Output Descriptors in Regus Core

Since Regus Core v0.17, there is support for Output Descriptors. This is a
...
- Followed by the actual key, which is either:
  ...
  - [WIF](https://en.regus.it/wiki/Wallet_import_format) encoded private keys may be specified instead of the corresponding public key, with the same meaning.
  - `xpub` encoded extended public key or `xprv` encoded extended private key (as defined in [BIP 32](https://github.com/RegusCrypto/rips/blob/master/rip-0032.mediawiki)).
...
- P2PKH addresses (base58, of the form `G...` for mainnet or `T...` for testnet). Note that P2PKH addresses in descriptors cannot be used for P2PK outputs (use the `pk` function instead).
- P2SH addresses (base58, of the form `X...` for mainnet or `S...` for testnet, defined in [BIP 13](https://github.com/RegusCrypto/rips/blob/master/rip-0013.mediawiki)).
- Segwit addresses (bech32 and bech32m, of the form `rgs...` for mainnet or `rgt...` for testnet, defined in [BIP 173](https://github.com/RegusCrypto/rips/blob/master/rip-0173.mediawiki) and [BIP 350](https://github.com/RegusCrypto/rips/blob/master/rip-0350.mediawiki)).
...
on Regus's OP_CHECKMULTISIG opcode. To support these, we introduce the
...
transactions related to the multisig. Lastly, it is not recommended to use anything other than a Regus Core descriptor wallet to serve as your
...
the entire path from the master down. [BIP174](https://github.com/RegusCrypto/rips/blob/master/rip-0174.mediawiki) standardizes this by
...
existing Regus Core wallets, a convenience function `combo` is
...
All RPCs in Regus Core will include the checksum in their output. Only
