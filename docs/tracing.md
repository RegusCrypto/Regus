# User-space, Statically Defined Tracing (USDT) for Regus Core

Regus Core includes statically defined tracepoints to allow for more
...
**Note**: Regus Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Regus Core
...
Multiple tools can list the available tracepoints in a `regusd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Regus Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/regusd
stap net        outbound_message 0x0000000000107c05 /src/regusd
stap validation block_connected  0x00000000002fb10c /src/regusd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Regus Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/regusd | grep NT_STAPSDT -A 4 -B 2
