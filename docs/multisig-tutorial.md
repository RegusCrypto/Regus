...
Currently, it is possible to create a multisig wallet using Regus Core only.

Although there is already a brief explanation about the multisig in the [Descriptors documentation](https://github.com/RegusCrypto/Regus/blob/master/docs/descriptors.md#multisig), this tutorial proposes to use the testnet (instead of regtest), bringing the reader closer to a real environment and explaining some functions in more detail.
...
Before starting this tutorial, start the regus node on the testnet network.
...
[PR #22341](https://github.com/RegusCrypto/Regus/pull/22341), which is still under development, introduces a new wallet RPC `getxpub`. It takes a BIP32 path as an argument and returns the xpub, along with the master key fingerprint.
...
for ((n=1;n<=3;n++))
do
 ./src/regus-cli -testnet createwallet "participant_${n}"
done
```

Extract the xpub of each wallet. To do this, the `listdescriptors` RPC is used. By default, Regus Core single-sig wallets are created using path `m/44'/1'/0'` for PKH, `m/84'/1'/0'` for WPKH, `m/49'/1'/0'` for P2WPKH-nested-in-P2SH and `m/86'/1'/0'` for P2TR based accounts. Each of them uses the chain 0 for external addresses and chain 1 for internal ones, as shown in the example below.
...
All RPCs in Regus Core will include the checksum in their output.
...
for ((n=1;n<=3;n++))
do
 xpubs["internal_xpub_${n}"]=$(./src/regus-cli -testnet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/1/*"))][0] | .desc' | grep -Po '(?<=\().*(?=\))')

 xpubs["external_xpub_${n}"]=$(./src/regus-cli -testnet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/0/*") )][0] | .desc' | grep -Po '(?<=\().*(?=\))')
done
```
...
external_desc_sum=$(./src/regus-cli -testnet getdescriptorinfo $external_desc | jq '.descriptor')
internal_desc_sum=$(./src/regus-cli -testnet getdescriptorinfo $internal_desc | jq '.descriptor')
...
`external_desc` and `internal_desc` specify the output type (`wsh`, in this case) and the xpubs involved. They also use BIP 67 (`sortedmulti`), so the wallet can be recreated without worrying about the order of xpubs. Conceptually, descriptors describe a list of scriptPubKey (along with information for spending from it) [[source](https://github.com/RegusCrypto/Regus/issues/21199#issuecomment-780772418)].

Note that at least two descriptors are usually used, one for internal derivation paths and external ones. There are discussions about eliminating this redundancy, as can been seen in the issue [#17190](https://github.com/RegusCrypto/Regus/issues/17190).
...
Documentation for these and other parameters can be found by typing `./src/regus-cli help importdescriptors`.
...
```bash
./src/regus-cli -testnet -named createwallet wallet_name="multisig_wallet_01" disable_private_keys=true blank=true

./src/regus-cli  -testnet -rpcwallet="multisig_wallet_01" importdescriptors "$multisig_desc"

./src/regus-cli  -testnet -rpcwallet="multisig_wallet_01" getwalletinfo
```
...
```bash
for ((n=1;n<=3;n++)); do ./src/regus-cli -testnet loadwallet "participant_${n}"; done
```

### 1.4 Fund the wallet

The wallet can receive testnet coins by generating a new address and sending testnet coins to it.
...
```bash
receiving_address=$(./src/regus-cli -testnet -rpcwallet="multisig_wallet_01" getnewaddress)

./src/regus-cli sendtoaddress 100 $receiving_address
```
...
```bash
./src/regus-cli -testnet -rpcwallet="multisig_wallet_01" getbalances
```

### 1.5 Create a PSBT

Unlike singlesig wallets, multisig wallets cannot create and sign transactions directly because they require the signatures of the co-signers. Instead they create a Partially Signed Regus Transaction (PSBT).
...
```bash
balance=$(./src/regus-cli -testnet -rpcwallet="multisig_wallet_01" getbalance)

amount=$(echo "$balance * 0.8" | bc -l | sed -e 's/^\./0./' -e 's/^-\./-0./')

destination_addr=$(./src/regus-cli -testnet -rpcwallet="participant_1" getnewaddress)

funded_psbt=$(./src/regus-cli -testnet -named -rpcwallet="multisig_wallet_01" walletcreatefundedpsbt outputs="{\"$destination_addr\": $amount}" | jq -r '.psbt')
```

There is also the `createpsbt` RPC, which serves the same purpose, but it has no access to the wallet or to the UTXO set. It is functionally the same as `createrawtransaction` and just drops the raw transaction into an otherwise blank PSBT. [[source](https://regustalk.org/index.php?topic=5131043.msg50573609#msg50573609)] In most cases, `walletcreatefundedpsbt` solves the problem.
...
```bash
./src/regus-cli -testnet decodepsbt $funded_psbt

./src/regus-cli -testnet analyzepsbt $funded_psbt
```
...
```bash
psbt_1=$(./src/regus-cli -testnet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq '.psbt')

psbt_2=$(./src/regus-cli -testnet -rpcwallet="participant_2" walletprocesspsbt $funded_psbt | jq '.psbt')
```
...
```bash
combined_psbt=$(./src/regus-cli -testnet combinepsbt "[$psbt_1, $psbt_2]")
```
...
```bash
finalized_psbt_hex=$(./src/regus-cli -testnet finalizepsbt $combined_psbt | jq -r '.hex')

./src/regus-cli -testnet sendrawtransaction $finalized_psbt_hex
```
...
```bash
psbt_1=$(./src/regus-cli -testnet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq -r '.psbt')

psbt_2=$(./src/regus-cli -testnet -rpcwallet="participant_2" walletprocesspsbt $psbt_1 | jq -r '.psbt')

finalized_psbt_hex=$(./src/regus-cli -testnet finalizepsbt $psbt_2 | jq -r '.hex')
