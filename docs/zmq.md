...
The Regus Core daemon can be configured to act as a trusted "border
router", implementing the regus wire protocol and relay, making
...
The ZeroMQ feature in Regus Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building regusd:
...
For instance:
    ...
    $ regusd -zmqpubhashtx=tcp://127.0.0.1:14615 \
               -zmqpubhashtx=tcp://192.168.1.2:14615 \
               -zmqpubhashblock="tcp://[::1]:14610" \
               -zmqpubrawtx=ipc:///tmp/regusd.tx.raw \
               -zmqpubhashtxhwm=10000

Each PUB notification has a topic and body, where the header
corresponds to the notification type. For instance, for the
notification `-zmqpubhashtx` the topic is `hashtx` (no null
terminator). These options can also be provided in regus.conf.
...
From the perspective of regusd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into regusd directly. Furthermore, no information is
...
using. Regusd appends an up-counting sequence number to each
