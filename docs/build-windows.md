...
Below are some notes on how to build Regus Core for Windows.

The options known to work for building Regus Core on Windows are:
...
* On Windows, using [Microsoft Visual Studio](https://visualstudio.microsoft.com). See [README.md](/build-msvc/README.md).
...
Note that for WSL the Regus Core source path MUST be somewhere in the default mount file system, for
example /usr/src/regus, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\regus`, for example:

    make install DESTDIR=/mnt/c/workspace/regus
