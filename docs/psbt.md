...
PSBT is an interchange format for Regus transactions that are not fully signed
yet, together with relevant metadata to help entities work towards signing it.
It is intended to simplify workflows where multiple parties need to cooperate to
produce a transaction. Examples include hardware wallets, multisig setups, and
[CoinJoin](https://regustalk.org/?topic=279249) transactions.

### Overall workflow

Overall, the construction of a fully signed Regus transaction goes through the
following steps:
...
- An **Extractor** produces a valid Regus transaction (in network format)
...
#### Multisig with multiple Regus Core instances
...
Regus Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `regus-cli` is used.
...
Later, when *V* REG has been received on *Amulti*, and Bob and Carol want to
