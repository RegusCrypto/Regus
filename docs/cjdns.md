# CJDNS support in Regus Core

It is possible to run Regus Core over CJDNS, an encrypted IPv6 network that
...
done outside of Regus Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Regus network and individual nodes.
...
before it will work with your Regus Core node. This is described in steps
...
## Run Regus Core with CJDNS

Once you are connected to the CJDNS network, the following Regus Core
...
When enabled, this option tells Regus Core that it is running in an
...
IPv6 local network. This helps Regus Core perform better address management:
...
CJDNS support was added to Regus Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `regus-cli -addrinfo` to see the
...
[docs/tor.md](https://github.com/RegusCrypto/Regus/blob/master/docs/tor.md) for
details.

## CJDNS-related information in Regus Core

There are several ways to see your CJDNS address in Regus Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `regus-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `regus-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `regus-cli help getnodeaddresses` for details.
