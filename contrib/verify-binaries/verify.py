...
"""Script for verifying Regus Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from regus.app and regus.app and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/RegusCrypto/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://regus.app"
HOST2 = "https://regus.app"
VERSIONPREFIX = "regus-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Regus community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"regus_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by regus.app
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the regus release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes regus.app lags behind regus.app.)')
    )
