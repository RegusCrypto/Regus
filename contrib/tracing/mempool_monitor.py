...
""" Example logging Regus Core mempool events using the mempool:added,
...
def main(regusd_path):
    regusd_with_usdts = USDT(path=str(regusd_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    regusd_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    regusd_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    regusd_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    regusd_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[regusd_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/regusd")
        exit(1)
