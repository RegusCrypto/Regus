# Bootstrappable Regus Core Builds

This directory contains the files necessary to perform bootstrappable Regus
Core builds.
...
To build Regus Core reproducibly with all default options, invoke the
...
documentation](/docs/release-process.md) for more context.
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Regus Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/regus-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/RegusCrypto/guix.sigs).
