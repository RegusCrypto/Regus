...
(
    ...
    case "$HOST" in
        *mingw*)
            make deploy ${V:+V=1} REGUS_WIN_INSTALLER="${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
            ;;
    esac

    # Setup the directory where our Regus Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built Regus Core to $INSTALLPATH
    case "$HOST" in
        *darwin*)
    ...
    (
        ...
        case "$HOST" in
            *mingw*)
                cp "${DISTSRC}/docs/README_windows.txt" "${DISTNAME}/readme.txt"
                ;;
            *linux*)
        ...
        # copy over the example regus.conf file. if contrib/devtools/gen-regus-conf.sh
        # has not been run before buildling, this file will be a stub
        cp "${DISTSRC}/share/examples/regus.conf" "${DISTNAME}/"
