...
**Regus Core's `configure` script by default will ignore the depends output.** In
...
The paths are automatically configured and no other options are needed unless targeting [Android](../docs/build-android.md).
...
- see [build-windows.md](../docs/build-windows.md#cross-compilation-for-ubuntu-and-windows-subsystem-for-linux)
...
options will be passed to regus's configure. In this case, `--disable-wallet`.
