...
def parse_config_into_reg_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Regus Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build-msvc/regus_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build-msvc/regus_config.h'), "w", encoding="utf8") as reg_config:
        reg_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='Regus-core msbuild configuration initialiser.')
    ...
    set_properties(os.path.join(SOURCE_DIR, '../build-msvc/common.init.vcxproj'), '@TOOLSET@', args.toolset)
    ...
    parse_config_into_reg_config()
    copyfile(os.path.join(SOURCE_DIR,'../build-msvc/regus_config.h'), os.path.join(SOURCE_DIR, 'config/regus-config.h'))
