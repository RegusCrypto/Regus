...
"""Regus test framework primitive and message structures
...
COIN = 100000000  # 1 reg in satoshis
...
# Objects that map to regusd objects, which can be serialized/deserialized
...
class CBlockLocator:
    ...
    def serialize(self):
        r = b""
        r += (0).to_bytes(4, "little", signed=True)  # Regus Core ignores the version field. Set it to 0.
...
class CTransaction:
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in regusd
            if (flags != 0):
                self.vin = deser_vector(f, CTxIn)
...
class msg_version:
    ...
    def deserialize(self, f):
        ...
        # But, unconditionally check it to match behaviour in regusd
...
class msg_headers:
    ...
    def deserialize(self, f):
        # comment in regusd indicates these should be deserialized as blocks
