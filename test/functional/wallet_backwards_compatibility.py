...
class BackwardsCompatibilityTest(RegusTestFramework):
    ...
    def test_v19_addmultisigaddress(self):
        ...
        # Copy the 0.19 wallet to the last Regus Core version and open it:
        shutil.copytree(
    ...
    def run_test(self):
        ...
        if self.options.descriptors:
            ...
            for node in legacy_only_nodes:
                # RPC loadwallet failure causes regusd to exit in <= 0.17, in addition to the RPC
                ...
                for wallet_name in ["w1", "w2", "w3"]:
                    assert_raises_rpc_error(-4, "Wallet file verification failed: wallet.dat corrupt", node.loadwallet, wallet_name)
        ...
        if self.options.descriptors:
            ...
            # Descriptor wallets appear to be corrupted wallets to old software
            node_v17.assert_start_raises_init_error(["-wallet=w1"], "Error: wallet.dat corrupt")
            node_v17.assert_start_raises_init_error(["-wallet=w2"], "Error: wallet.dat corrupt")
            ...
            node_v17.assert_start_raises_init_error(["-wallet=w3"], "Error: Error loading w3: Wallet requires newer version of Regus Core")
        ...
        for wallet_name in ["w1", "w2", "w3"]:
            if self.options.descriptors:
                node_v16.assert_start_raises_init_error([f"-wallet={wallet_name}"], f"Error: {wallet_name} corrupt")
            else:
                node_v16.assert_start_raises_init_error([f"-wallet={wallet_name}"], f"Error: Error loading {wallet_name}: Wallet requires newer version of Regus Core")
        ...
        for node in descriptors_nodes if self.options.descriptors else legacy_nodes:
            ...
            # Make sure this wallet opens with only the migration warning. See https://github.com/RegusCrypto/Regus/pull/19054
